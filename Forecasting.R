...
#red is predict black= observation
...
# which is the best model , the less least square is the best model.
...
x <- ts(USAccDeaths,start=1973,freq=12)
x.SESmodel <- HoltWinters(x, beta=FALSE, gamma=FALSE)
x.DESmodel <- HoltWinters(x, gamma=FALSE) # important
x.HWmodel <- HoltWinters(x)
